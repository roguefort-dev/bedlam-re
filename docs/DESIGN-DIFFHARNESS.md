...
| watch | address (EXW) | EXD alias (D162, §5i) | layout | ledger anchor |
|---|---|---|---|---|
| weapon-anim bank | 0x4c71f4 | 0x980d4 (§5c) | 400 × 0x36 {type w@+0, owner, target, tick, xyz Q13, v, class, arc, trail link} | §8 row "weapon-anim tick" [verified] |
| projectile bank | 0x4cc654 | 0x10e174 (§5c) | 50 × 0x22 {type w@+0, xyz Q13, v} | §8 row "projectile tick" [verified] |
| mortar trail bank | 0x4e66b8 | 0x91574 | 20 × 0x68 {active, ring&7, 8 × xyz} | §8 row "mortar smoke-trail bank" [verified] |
| critter bank | 0x4cff98, count cell 0x46cc2c | 0x10e81c, count cell 0x1194dc | count × 0x7E (state+0x0C, hp+0x06, kind+0x00, presence+0x24) | §8 row "critter-actor controller" [verified] |
| POI/personnel bank | 0x4dabdc, count cell 0x46cbf0 | 0x971d4, count cell 0x119580 | count × 0x1E {active, state, heading, timer, xyz} | §8 row "POI/personnel controller" [verified] |

### T3 — effects, debris, rings, objectives

| watch | address (EXW) | EXD alias (D162, §5i) | layout | ledger anchor |
|---|---|---|---|---|
| debris stager | 0x476fbc | 0x93064 | 128 × 0x30 {active, xyz, seq, kind, phys, delay+0x24, param, seq-table} | §7j.7 item 5; §7j.11 kind table [verified] — the S1 "2k start-delay" watch |
| effect rows | 0x4cec38 | 0x9d534 | 80 × 0x20 (LRU-aged) | §8 row "effect-row spawner" [verified] |
| rising-debris bank | 0x4cf638 | 0xa1684 | 80 × 0x1E {xyz, v, group, active, delay+0x1A, frame} | §8 row "effects-bank stager" [verified] |
| platform/blast bank | 0x4eb638 | 0x8c284 | 32 × 0x14 {x, y, z, age, frame} | §8 row "robot-death blast bank" [verified] |
| splash records | 0x4e9778 | 0x107774 | 250 × 0xA {x, y, z, delay, age} | §8 row "splash records" [verified] |
| arrival/ride records | 0x4dcdb8 | 0x10da48 | 45 × 0x24 {active, marker, dest, countdown, robot} | §8 row "arrival ride tick" [verified] |
| door rects | 0x4dcae8 | 0x92c64 | 45 × 0x10 | §8 rows "door-rect list boundary", "door open/close" [verified] |
| delayed trigger timers | 0x4ea828 | 0x91d94 | 32 × 0x18 | §8 row "delayed trigger timers" [verified] |
| pod ring | 0x4e64c0 | 0x8d314 | 12 × 0x1C {active, phase, x, y, alt, group, dwell} | §8 rows "dropship ring banks", "pod spawner" [verified] |
| exit ring | 0x4e662c | 0x108138 | 5 × 0x1C (same layout) | §8 row "exit/threat slots" [verified] |
| dropship frame | 0x4e6610 | 0x1081c4 | 1 × 0x1C | §8 row "dropship deployer" [verified] |
| objective slots | 0x4eaaee, resolver phase 0x46cd00 | 0x8c182, phase 0x1194cc | 6 × 0x20 {remaining, type, status, quota} | §8 row "mission-objective resolver" [verified] |
| escape counters | 0x4eba0c / 0x4eba10 | 0x107674 / 0x107680 (0xC apart — EXW pair is adjacent) | u32 ×2 | §7j.19 (CLOSED census) [verified] |
| tile-claim bank | *(0x46af58) | *(0x119564) (both channels pointer-indirect) | 10000 B | §8 row "tile-claim bank" [verified] |
...
**The D162 subset-form extraction arms (2026-08-26, this
follow-up unit; RE provenance per row: critter §7j.17/§7j.42 +
the `critter.rs` field docs, effect rows §7j.24/5, debris
§7j.11/5, splash §7j.10 — all EXW-pinned, EXD twins per §5i):**
the four rows whose E canonical record is a SUBSET of the guest
record now carry O1/O2 extraction arms — the normalizer walks the
GUEST full span and projects E's modeled fields at the guest
offsets, so the rows compare cross-channel (the D87 field-map
class; fields a channel cannot source stay ABSENT per §6 — none
remain on these four). dropship-frame (the fifth E-emitting T3
row) is NOT subset-form — its canonical record IS the full guest
0x1C craft record field-for-field (§7j.40/6) — and landed its
identity arm with D164: the O1 arm reuses the E-side field walk
verbatim (the tile-word-grid/platform-strength identity
precedent), the O2 (and via it, O3) alias list takes the row
(same layout, EXW-pinned 0x4e6610 / EXD 0x1081c4), so it compares
cross-channel — ZERO E-only T3 rows remain.

| row | guest span (O1 plan) | the projection (guest offset → canonical field) | notes |
|---|---|---|---|
| critter-bank (T2) | `$critter_count*0x7E` at 0x10e81c (count cell 0x1194dc; EXW 0x4cff98/count 0x46cc2c) | 23 fields, count = span/0x7E (cap 350 = 0xAC44/0x7E): kind w@+0x00, species w@+0x02, attacker i16@+0x04, hp i16@+0x06, mode w@+0x0C, anim w@+0x0E, heading d@+0x10, impact_x d@+0x1C, impact_y d@+0x20, presence w@+0x24 (zero-ext), target_x d@+0x2A, target_y d@+0x2E, target_z d@+0x32, x d@+0x36, y d@+0x3A, z d@+0x3E, home_x d@+0x42, home_y d@+0x46, death_ctr d@+0x52, countdown w@+0x56 (zero-ext into the canonical i32), facing w@+0x72, target_robot i16@+0x7A, fuse w@+0x7C | every canonical leaf sources from the guest — ZERO field gaps; `count` classed STRUCTURAL like every bank count |
| effect-rows (T3) | `80*0x20` at 0x9d534 (EXW 0x4cec38) | count = 80 (the fixed LRU bank): age w@+0x00, x d@+0x02, y d@+0x06, z d@+0x0A, cos d@+0x0E, sin d@+0x12, ttl d@+0x16, id w@+0x1A | full-bank row — E emits all 80; `count` STRUCTURAL |
| debris-stager (T3) | `128*0x30` at 0x93064 (EXW 0x476fbc) | count = 128: active u8@+0x00, kind d@+0x1C, delay d@+0x24, seq d@+0x18 | the +0x18 seq is the DUAL field the ENGINE SPLITS (§7j.44: E keeps the LRU-eviction role as the global staging counter `debris_seq`, the walk-cursor role as `anim`); the projection carries the guest's raw +0x18 — the VALUE diverges from E's counter by construction and stays silent only because the row is T3 (never bit-compared); if the row is ever re-tiered, this offset pair is the known encoding difference |
| splash-records (T3) | `250*0xA` at 0x107774 (EXW 0x4e9778) | count = 250: x/y/z/delay/age u16 @+0x00..+0x08 — the guest stride exactly | identity projection, count synthesized from the fixed bank |

differ-coverage consequence: the differ_gate S4/S7/S8 lanes drop
their debris/splash/critter/effect E-only row-level findings
(3 → 1 each: move-target-words remains the one E-only row);
D164 drops S6's dropship finding too (1+1 → 1 — move-target-words
is now the ONLY E-only row, row- or field-level, on every
scenario). The fabricated-guest (`inv_frame`) arms place each
canonical field back at its guest offset — presence/countdown
narrow word↔i32 at the word, the debris table byte zero-extends
into the +0x2C pointer word (E models the seq-table INDEX, not
the pointer); dropship-frame needs NO projection at all — the
canonical 28-B record rides identity through the scalar
catch-all.
...
12. **W12 — scenario depth S3–S8** as producer families land in-engine
    ...
    **S4 LANDED 2026-08-22 (D105):** grammar v1.4 adds the `destroy`
    ...
    splash-records 10-B FULL bank — the T3 pair's EXD aliases were
    pinned by D162 (§5i) and the rows now carry the subset-form
    O1 extraction arms: fabricated through the guest-span
    projections, compared cross-channel).
    ...
    and the D162 subset-form extraction arms of 2026-08-26
    fabricate the debris/splash pair through the guest-span
    projections, dropping it to 1 — move-target-words —
