...
0x1C craft record field-for-field (§7j.40/6) — and keeps its
E-only differ-coverage state until its identity arm lands.
...
S6 keeps its dropship finding (1+1). The fabricated-guest
(`inv_frame`) arms place each canonical field back at its guest
offset — presence/countdown narrow word↔i32 at the word, the
debris table byte zero-extends into the +0x2C pointer word
(E models the seq-table INDEX, not the pointer).
