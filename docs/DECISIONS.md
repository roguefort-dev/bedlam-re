...
## D212 — 2026-08-28: P6/QoL `p6-volume-mixers` — the per-bus volume selection (a platform knob OUT of ModeConfig with NO purist arbitration, re-anchored on the original's ONE shared master bus: the gain applies at the shell feed site on the device-bound copy only, composed multiplicatively — each knob alone is the original's own whole-mix stepper; the bounded runtime key set is platform-only and dead to both schemes), wired as the NINTH P6 required gate

CONTEXT: PLAN §6 P6 QoL names the list "window modes, vsync
control, volume mixers, save slots + metadata + opt-in autosave".
Vsync landed as D208, window modes as D210; this unit lands VOLUME
MIXERS. RE FIRST: the bus split is re-anchored in a committed
artifact BEFORE the implementation (docs/RE-EXW-MUSIC sec 7 — a
COLLECTION of already-verified facts, no new claims): the shipped
EXW has ONE shared master bus. The mission-shell stepper
(RE-EXW-INPUT sec 5) moves g_music_volume 0..100 in ±5 steps and
applies `vol>>1` through FUN_0044c630 into the master word
004ee9b4 (scale 0..127); that master scales EVERY voice — music
script notes AND sfx RAW voices ride the same SubVoiceStart master
product — and the only sfx-side separation in the original is the
registry sfx-master-gate (0x4ede58, D134), an on/off MUTE, never a
gain. So the original's "music volume" slider is a WHOLE-MIX
master, and a per-bus music/sfx selection is a MODERN platform
addition whose default must equal the shipped single-bus mix
exactly.

DECISIONS:

1. The volume mixers are a PLATFORM KNOB OUT of `ModeConfig`
   (D200 layering) with NO purist arbitration: audio is
   presentation bucket (D17 b — the mixed stream never reaches a
   hash), the knob never consults the mode, and both pacing arms
   accept it identically. `VolumeLevel` (0..=100 percent, the
   original's own UI domain, clamped like the original) x
   `VolumeMixers { music, sfx }` (default `SHIPPED` = both FULL)
   in bedlam-shell/src/audio.rs, carried as
   `WindowOptions::volume`; the binary's `--music PCT`/`--sfx PCT`
   select the starting levels (noted + ignored headless, the
   --fullscreen posture).
2. THE GAIN APPLICATION SITE is the shell audio path ONLY — the
   `AudioFeed::fill_from` watermark site scales the DEVICE-BOUND
   copy after `GameHost::render_audio`, Q8 integer math (unity 256
   at the shipped default = an EXACT bit-identical passthrough,
   pinned by `shipped_mixers_feed_the_engine_stream_bit_exactly`;
   gain never exceeds unity, so no clamp can be owed). Because the
   engine bus is faithfully UN-SPLIT (one
   `Mixer::set_master_volume` scaling all voices + the live-gain
   PCM stream), the per-bus pair composes MULTIPLICATIVELY on the
   shared stream — the honest composition: each knob ALONE
   reproduces the original's own whole-mix stepper behavior
   exactly, and a true engine-side bus split (the instrument
   domain is already disjoint, D42.7) stays future engine work,
   out of this unit's bounds (no engine change; bedlam-shell
   only). The engine's mixed parity stream (the audio determinism
   gate), the sim and every hash are untouched under ANY knob
   setting — pinned by `volume_mixers_never_touch_the_engine_
   stream` (host renders knob-invariant before AND after
   non-unity fills) and `volume_selection_never_touches_the_sim_
   config` (bit-identical derived SimConfig), and checked
   end-to-end: the headless smoke stays at the recorded baseline
   (scene 696adb1cd110e062, parity cce30c983b97b16d, audio
   110400/158092) under `--music 50 --sfx 0`.
3. THE BOUNDED RUNTIME KEY SET (the F11 posture): PageUp/PageDown
   step the music bus ±5 and BracketRight/BracketLeft the sfx bus
   — the original's own step and 0..=100 clamp (RE-EXW-INPUT sec
   5) — as PLATFORM keys OUTSIDE both control schemes, intercepted
   in the event handler BEFORE the mapper so they NEVER reach
   `ShellInput`; pinned by
   `volume_keys_are_platform_only_and_dead_to_both_schemes` (the
   four keys and nothing else, unbound in the modern default table
   and dead in the classic fixed scheme — the ORIGINAL volume keys
   Up/Down deliberately stay scheme keys, not platform keys). A
   mid-run adjustment applies to FUTURE fills only
   (`runtime_mixers_adjust_applies_to_future_fills_only`); the
   adjustment note is a stderr line, never fatal.
4. GATE: `p6-volume-mixers` wired as the NINTH P6 required gate —
   command = bedlam-shell --lib, --release --locked --offline,
   hermetic. Test surface = the ONE selection (domain, Q8 edges,
   composition, the shipped-default bit-exact pin, the invariance
   pins, the runtime-adjust pin, the key set), never the feature
   cross-product; the catalog stays EMPTY (a plan-named QoL unit
   is not a catalog entry).

VERIFIED (first-hand, this unit): bedlam-shell --lib 75/0 (+10: 7
audio — the level domain/Q8 conversion, the shared-bus
composition, the unity bit-exact identity sweep, the scale/mute
math, the shipped-default engine-stream pin, the knob-invariance
pin, the future-fills-only pin; 3 window — the platform-only key
set dead to both schemes, the original step/clamp, the sim-config
invariance; was 65/0 + 1 pre-existing ignored); the binary
--help/--music/--sfx wiring checked first-hand (help text, the
headless ignore note, the baseline hashes unchanged); controls
green: canonical_dump_gate 13/13, determinism 4/4 + bedlam-core
12/12, zone_mission_parity 5/5 (ZERO canonical-chain movement);
fmt + clippy clean on bedlam-shell (the one `let mut opts` test
...
## D238 — 2026-09-02: gates/completion `required-gates-v2-contract` — THE AUDITED FALSE-COMPLETION REVOCATION: none of the 37 commands in docs/required-gates.toml is a true product-path gate (all are supporting/static/paperwork/synthetic/corpus-required/infrastructure evidence — preserved intact, commands unchanged), P0-P3 carried zero gates at all, P5's 37 mission greens were granted on replay/oracle parity with no natural playthrough (headless injects SceneAction::MissionComplete at engine/bedlam-shell/src/headless.rs:125 while production MissionScene::tick at engine/bedlam-game/src/mission.rs:988 returns no outcome), P6 graded inert API surfaces, P7 graded package/workflow definitions — so the manifest moves to schema required-gates-v2 where EVERY gate carries a validated `evidence` classification (product | supporting | static | paperwork | synthetic | corpus-required | infrastructure; all 37 existing gates classified non-product, zero relabeled product), phase statuses are vocabulary-checked (pending | engineering-green | green), a `green` phase is structurally rejected unless it wires at least one product gate (the validator refuses the manifest, fail-closed), plan completion requires every phase product-green, and the old v1 schema is refused outright so the old manifest can never validate again; per-gate classifications: supporting = trigger-address, dependency-spikes, diffharness-plumbing, p6-present-loop-wiring, p6-uncapped-present-mode, p6-enhanced-native-render, p7-cdda-user-supply, p7-steamdeck-default; static = s0-dispositions, p6-modeconfig-seam, p6-timing-lock-surface, p6-control-scheme-surface, p6-high-refresh-interpolation, p6-window-modes, p6-volume-mixers, p6-save-slots, p6-scaling-options; paperwork = p4-machine-verdict, p5-zone-gate-scaffold, p6-modernization-scaffold, p6-hd-asset-research, p7-ports-scaffold, p7-ci-artifacts, p7-flatpak-manifest, p7-windows-installer, p7-macos-universal2-ci; synthetic = p6-frame-pacing-benchmark; corpus-required = scripted-slice-scenes, p5-zone-a..g; infrastructure = gates-validator, env-probe. STATUS REVOCATION: P0-P6 green -> pending, P7 green -> engineering-green (its 7 engineering gates remain green as ENGINEERING evidence; the bounded --phase run now emits schema phase-verdict-v2 with an explicit product_complete=false, so even a legacy .state/P7-COMPLETE path cannot carry a completion claim — the old phase-complete-v1 markers and .state/*-COMPLETE files stay on disk as non-authoritative residue: nothing reads them, nudge.sh already discards PLAN-COMPLETE every pass, and the validator derives everything from tracked HEAD content, pinned by test). LEDGER REVOCATION: docs/P5-MISSION-LEDGER.toml moves to schema p5-mission-ledger-v2 with disposition vocabulary unproven|green — all 37 rows reset to `unproven` (machine-readable), the v1-era parity evidence PRESERVED per-row in a new `supporting_evidence` field citing the p5-zone-{a..g} gates; checker rules v2: citations must name defined gates, a wired p5-zone-{a..g} gate must be cited by its zone's rows (LINKAGE — the v1 rule that demanded zone green was exactly the false-completion coupling and is replaced), a green row must cite a product gate, P5 green still requires 37/37 green; docs/P5-ZONE-GATES.md §3/§4 updated to v2 with the v1 landing rule kept as history. REPORT: schema required-gates-report-v2 with per-gate evidence, product_gates, phase_product_coverage, and why_incomplete. TESTS (all focused, no 37-gate battery): tools/test-validate-required-gates.py 31/31 (v1-schema rejection, evidence-class validation, status vocabulary, green-phase-without-product-gate rejected for ALL SIX non-product classes, the old-manifest shape with all statuses green rejected, all-engineering-green never completes, the only completing shape is all-phases-green-with-product-gates, engineering-green phase verdict product_complete=false even into a *-COMPLETE filename, legacy .state markers non-authoritative byte-for-byte); tools/test-p5-zone-ledger.py 22/22 incl. the deliberate real-repo re-baseline 0/37 green + 37 unproven (the fingerprint discipline, same commit); check-p6-behavior-catalog OK + 30/30; check-p7-ports-map OK + 29/29 (P7 engineering-green satisfies R6 vacuously); test-p6-hd-asset-research 27/27; the p5 ledger checker OK over the read-only corpus with MANIFEST.sha256 clean before and after. QUEUE SEEDING (the .state/NEXT.md rewrite): seven dependency-ordered READY machine tracer units — scheduler-gate-cache-v1 (per-gate basis fingerprint cache ending the ~30min/~12GB full re-runs), queue-synthesis-v1 (deterministic failed-product-gate -> READY synthesis with product-vs-infrastructure failure typing; the designated source for the later 37-mission backlog expansion), shell-input-seam-v1, shell-controller-v1 (platform-neutral ShellController shared by winit/headless/tests), menu-journey-v1 (the FIRST product gate), mission-outcome-v1 (mission-owned outcome resolver), zonea-trace-v1 (Zone A production trace) — dependencies encoded by queue order under the pinned concurrency ceiling of 1 and lowest-ordinal claiming (nudge.sh takes the first unclaimed READY), so no WAITING-AUTOMATIC probe is required: none of the seven waits on an external machine event, each is deterministic build-and-test work. LIVELOCK CLOSURE: the controller's completion branch runs only on REQUIRED-QUEUE-EMPTY, so with the queue nonempty the intentionally-red product state drives item 1 normally and can never beacon completion-missing -> watchdog repair churn. RESIDUAL DRIFT NOTED, deliberately not touched here: AGENTS.md's Completion paragraph still describes the v1 condition ("every P0-P7 gate green"); under v2 the validator additionally requires green statuses with wired product gates — the operator-owned contract file is left for the owning operator to amend after review. Do not read any pre-D238 green status, P*-COMPLETE marker, or gates report as product completion.


## D239 — 2026-09-02: scheduler `scheduler-gate-cache-v1` — THE PER-GATE FINGERPRINT VERDICT CACHE: tools/validate-required-gates.py gains an OPT-IN `--gate-cache DIR` accelerator (default absent = byte-identical behavior for every existing caller: the controller's sealed completion runs, the gates-validator gate, and every worker invocation; NO gate command changes, no engine change, no manifest edit) so the D238-era full-battery re-runs (~30min, ~12GB: every controller tick and every repeated worker validation re-executed all 37 gates even at an UNCHANGED HEAD) collapse to instant cache hits on repeat validation. THE BASIS FINGERPRINT (per gate, sha256 over length-framed fields): schema tag required-gate-cache-v1, the HEAD commit, the whole tracked-tree content fingerprint, the required-gates manifest sha256, the MANIFEST.sha256 corpus digest, the validator's own bytes, and the gate's own slice — id, evidence class, timeout_seconds, the full command argv arrays, the digest of every tracked_paths/corpus path, the digest of every command argument that names a tracked script (the command_arrays() predicate mirrored), writable, and depends. Any tracked change anywhere re-runs every gate BY DESIGN (the scheduler's pain was repeats at an unchanged HEAD; per-gate inputs are an additional binding, not a scoping mechanism). THE SEMANTICS: verdicts cache under the dir keyed by sha256(gate id).json with the id stored inside the entry; ONLY green verdicts of gates that actually executed their commands are remembered (red gates and dependency-failed gates never write, and dependency-failed gates never even consult the cache — dependency consequences stay live); a hit requires an EXACT entry shape {schema,id,basis_sha256} as a regular non-symlink file of at most 4096 bytes whose id and basis match this run's computation — anything else (missing, malformed, corrupt, foreign, oversized, symlinked) fails closed to a re-run and is replaced by the honest fresh entry; a hit replays the green command verdicts byte-identically (a remembered green implies rc=0 for every command), so reports are deterministic across miss→hit and the per-command-boundary basis revalidation simply has no boundaries to recheck on an all-hit run. THE PATH CONTRACT: a cache path inside the invocation root must be gitignored, contain no tracked content, and never traverse a symlink (the ensure_writable_dir discipline — the cache can never become, cover, or fabricate tracked evidence; violations fail closed and loudly, never silently disabling); a path outside the root is allowed host-side runtime state (the controller/complete-from-head shape: bases are content-keyed, so a persistent host cache serves sealed read-only checkouts at the same HEAD, with reads working and writes skipped best-effort); the canonical in-repo path is /gate-cache (gitignored D239, no trailing slash so check-ignore matches before the directory exists). TRUST BOUNDARY: the cache is host runtime state with the same trust as .state — an attacker with write access to it can already change anything the harness reads; the fail-closed entry contract refuses every malformed/foreign/poisoned shape a test can plant, and the gitignore enforcement keeps the cache out of tracked evidence permanently. VERIFIED first-hand: tools/test-gate-cache.py 15/15 (disabled-by-default never writes or reads, miss-remembers-green, deterministic hit with byte-identical reports and untouched entry inode/mtime, hit-proven by forced-miss rewrite, basis-change re-runs on gate-input AND unrelated tracked changes, corrupt-entry and oversized-entry fail-closed-to-re-run with repair, six poison shapes refused, symlinked-entry refusal, dirty-tracked-path rejection despite cache, dependency failure writes nothing and stays red across runs, phase-run reuse, out-of-root host cache, and the three in-root path refusals); tools/test-validate-required-gates.py 34/34 unchanged; py_compile clean; MANIFEST.sha256 clean before and after (the suites are hermetic over fixtures — no corpus read); and the real-repo bounded --phase P7 smoke at the landing commit (first run executes all 7 gates, second run all hits, reports byte-identical).


## D240 — 2026-09-02: scheduler `queue-synthesis-v1` — DETERMINISTIC FAILED-PRODUCT-GATE SYNTHESIS (THE CONTROLLER HOOK): on a REQUIRED-QUEUE-EMPTY parse whose sealed full-battery required-gates validation FAILED, the controller's completion branch (tools/nudge.sh, after the D234 basis-change classifier, before the completion-missing beacon) now calls the NEW tools/nudge-state.py `synthesize-product-work`, which deterministically publishes READY queue items synthesized from the failed report — and ONLY from product-class failure shapes: (1) a wired evidence="product" gate whose OWN evidence ran and is red (recorded command verdicts with a nonzero exit — a failing gate with no commands of its own is a dependency consequence and never synthesizes; the synthesized item cites the gate id, the first red command's argv when every character is grammar-safe + automation-lint-clean and at most 400 chars, else "argv withheld", plus the exit code), and (2) a phase wiring zero product gates (phase_product_coverage == 0 — product completion is structurally impossible under required-gates-v2 until a gate exists; a synth-wire-<phase> wiring item). EVERYTHING ELSE REFUSES WITHOUT TOUCHING THE QUEUE (byte-identical queue, refusal reason logged, the branch falls through to the structured completion-missing beacon for watchdog repair exactly as before): red non-product gates (supporting/static/paperwork/synthetic/corpus-required/infrastructure never synthesize product work), dependency-blocked product gates whose failure root is not a red product gate, error-shaped reports (validator/sandbox/corpus/harness), stale HEAD bindings, bounded phase-run reports, non-failed/absent-coverage shapes, non-empty active queues, and claim residue. Publication is fail-closed: SAFE_ID identities (synth-repair-<gate> / synth-wire-<phase>, collision-checked), the candidate queue validated by the strict grammar (tools/nudge-free-items.py) BEFORE replace_publish (which takes the queue lock), and every synthesized item carries a BOUNDS line — no synthesized item ever asserts a phase status, only the validator flips phases; the item is ordinary claimable work on the next tick and is the designated mechanical source for the later 37-mission product backlog. THE HERMETIC ENV FIX CLASS (discovered live by worker c5b582e3, finished by watchdog repair 1204180): agent-spawning suites run from INSIDE a nudge worker session inherited NUDGE_OWNER_FD/NUDGE_CLAIM_IDENTITY (the wrapper's claim-owner-exec exports), so the agent under test skipped its own claim-owner-exec re-exec (nudge-agent.sh checks only the VALUE "8") and died at launch preflight claim-invalid against the fixtures — reproduced first-hand under a simulated worker environment (NUDGE_OWNER_FD=8 + foreign NUDGE_CLAIM_IDENTITY + foreign NUDGE_QUEUE_LOCK_HELD); test-nudge-claims, test-waiting-automatic, and test-nudge-controller strip the wrapper vars (the interrupted worker's fixes), and this repair extends the identical guard to test-lock-v2-adversarial (15 categories), test-automation-failure-watchdog (1), test-final-hardening-red (1), test-autonomy-remaining-gaps (2 leak categories), and test-reviewer-security-red (2 leak categories) — all five proven red under simulation before the guard and restored to their clean-env verdicts after it; test-nudge-transport-markers verified green in the real worker context (no guard needed); production units launch through systemd-run with a clean environment, so nothing production changes. PRE-EXISTING SUITE ROT RECORDED (NOT this unit's regressions, proven at HEAD b7af042 in an isolated worktree with this change absent): test-autonomy-remaining-gaps case "completion ignores worker marker and controller validates empty queue" (its fixture still writes a required-gates-v1 manifest the D238 validator refuses outright) and test-reviewer-security-red cases "empty queue validates before connectivity...", "completion materializes the real external corpus...", "validator runs with no network...", "isolated real Cargo gate writes only through...", "completion rejects replaced proof...", and "completion acceptance freshly resnapshots claims..." (the same D238 v1-fixture rot) — all red at HEAD and byte-identically red with this change; queued as autonomy-suite-rot-v1 behind the tracer chain, NOT fixed here (one bounded unit per run). VERIFIED first-hand: tools/test-queue-synthesis.py 15/15; tools/test-nudge-controller.sh PASS end-to-end including the new live-hook tests (13: a red wired product gate publishes the READY item, logs the publication, emits no beacon, starts no watchdog, leaves the queue RUNNABLE 1, and a stale-heartbeat tick spawns a worker for the synthesized item end to end; 14: a red infrastructure gate refuses, beacons completion-missing with the structured artifact, starts the watchdog, never touches the queue, spawns nothing); tools/test-nudge-claims.sh PASS; tools/test-waiting-automatic.sh PASS; tools/test-nudge-queue.sh PASS; tools/test-validate-required-gates.py 34/34; the five guarded suites restored to their clean-env verdicts under the simulated worker environment; py_compile + bash -n clean; no engine change, no manifest edit, no corpus read (MANIFEST.sha256 clean).

## D241 — 2026-09-06: live mission pointer alignment

The interactive operator resumed product playtesting with the legacy workers
still paused, per the user's instruction to work alone. Window input now steers
the active Mission cursor as well as the Title cursor to the visible pointer
mapped through the presentation rectangle. This fixes a platform-coordinate
mismatch without changing mission click rules or canonical replay inputs.
The real-corpus MAP-button regression failed at (320,240) before the fix and
passes at (560,450) afterward; the rebuilt game opened the map on a visible
