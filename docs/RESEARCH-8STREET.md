...
4. **.MRS event encoding** — per-channel chunk/event opcodes (the sequencer logic is reconstructable from [ASM] 4897–5563 but no symbolic names exist for event words). Needed for a standalone music player.
...
7. **CONFIG.BDL (61 B, root)** — layout unknown; reconstruction ignores it. DOS installer artifact?
