...
4. **.MRS event encoding** — per-channel chunk/event opcodes (the sequencer logic is reconstructable from [ASM] 4897–5563 but no symbolic names exist for event words). Needed for a standalone music player. **PARTLY ANSWERED 2026-08-17**: EXW loader chain + container layout now EXW-anchored and byte-verified against all 5 shipped files (load_midi=FUN_00403642, load_mrs=FUN_00403827, load_mrw=FUN_004038c6, LoadFile=FUN_0041cc7f; header u16 W0/W1 + size array + 3 tables + data at 4+4W0+6W1W0) — see docs/RE-EXW-MUSIC.md. Still open: event-word opcodes (needs the sequencer pump consumer) + .MRW internal layout.
