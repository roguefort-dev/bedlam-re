...
- **Deathmatch**: `game_mode==2` ⇒ mission 6/7 of a zone (MISSION6.*, MISSION7.*). These have 16-byte NME files (empty spawn lists); the shipped MISSION6.BIN variants in ZONEB/ZONEE (and ZONED's MISSION5.BIN) are runtime-dead editor residue — the game always loads the zone-level BIN ([ASM] 97139–97144; [DATA] NME sizes; EXW verdict RE-EXW-SIM §7c.9).
...
4. `get_current_mission_path()` then `load_tot_dat_cgr_bin_min_pad` ([ASM] 98746) loads, in order: **MISSION\<n\>.TOT → MISSION\<n\>.DAT → MISSION\<letter\>.CGR → MISSION\<letter\>.BIN → MISSION\<letter\>.MIN → MISSION\<letter\>.LNG (German) / .LNK (else)**; parses TOT header, DAT z-offsets, then **MISSION\<n\>.PAD**.
   - Note the split: per-mission files are TOT/DAT/PAD (+NME/TRT/MRK/POS/BDG below); per-zone shared files are CGR/BIN/MIN/LNK/LNG — and the BIN is **ALWAYS the zone-level bank** [EXW-verified 2026-08-28, RE-EXW-SIM §7c.9, answering OPEN QUESTIONS #3]: the shipped MISSION6/MISSION5 variant banks (ZONEB M6, ZONED M5, ZONEE M6) are runtime-dead editor residue, NOT loaded "because var_buffer is used first" as this gloss once speculated.
...
3. **MISSION\<n\>.BIN override rule** — **ANSWERED 2026-08-28 [EXW+DATA]**: there is NO override. EXW's `build_mission_paths` (FUN_00446670c, walked whole) builds path2 (the base for `.CGR/.BIN/.MIN/.LNG/.LNK`) as `EDITOR\ZONE{L}\MISSION{L}` unconditionally — zone letter twice, no itoa, no conditional — and the only two `.BIN` concat sites (FUN_0041dc5a @0x41dcbc, brief twin FUN_0044661b @0x446644) both use it; a whole-image string census finds no other mission-path builder, and the EXD twin agrees. The shipped `ZONEB/MISSION6.BIN`, `ZONED/MISSION5.BIN`, `ZONEE/MISSION6.BIN` (which differ from the zone banks, ZONEB M6 ≡ ZONED M5 byte-identical) are editor-side residue never opened at runtime; the 8street reconstruction's "loaded only when the mission has its own" gloss (§1.1) was wrong. Full evidence chain: docs/RE-EXW-SIM.md §7c.9.
