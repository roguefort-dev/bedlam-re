...
**C. The EXD twins + the span-form asymmetry.** RE-EXD-MAP §5b [verified]: EXW **0x4eddec (W) / 0x4eddf0 (H)** ⟷ EXD **0x1074b8 (W) / 0x10748c (H)**; EXW plane-stride 0x4eddf4 ⟷ EXD w·h product cell 0x1074e4. THE GAP IS ASYMMETRIC between the builds: the EXD pair sits **0x2c apart with h LOW** (h 0x10748c < w 0x1074b8 — the O1 row's 0x30 span reads h@+0x00, w@+0x2c), while the EXW pair sits **0x24 apart with w LOW** (w 0x4eddec < h 0x4eddf0). The port shuffled the intervening neighbors, so the O2 raw form is NOT the O1 form with relabelled fields — the field order is REVERSED relative to address order.

**D. THE W11 PIN — the O2 capture form.** One contiguous span covering EXACTLY the two cells, mirroring the EXD precedent (one contiguous read, both cells, product cell excluded both sides): **base 0x4eddec, len 0x28 (= 0x24+4) — w @+0x00, h @+0x24**. `normalize_o2_row`'s static-map-wh arm parses this into the canonical (w, h) fields (previously ZERO fields pending this pin — the deliberate W11-prep gap this unit closes); the future W11 capgen emits the span. The differ_gate O2 fabrication emits the same 0x28 form and the E-vs-O2 cross now compares the row CLEAN (the pre-pin zero-field arm would have shown the row as 2 field-level coverage gaps).
