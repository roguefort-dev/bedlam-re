...
pub fn blit_indexed(
    ...
    let rows = (src.len() / w).min(src_h as usize);
    for row in 0..rows {
        let dy = dst_y + row as i32;
        if dy < 0 || dy >= CANON_H as i32 {
            continue;
        }
        let src_row = &src[row * w..row * w + w];
        let dst_base = dy as usize * CANON_W as usize;
        for (col, &index) in src_row.iter().enumerate() {
            let dx = dst_x + col as i32;
            if dx >= 0 && (dx as u32) < CANON_W {
                frame.indices[dst_base + dx as usize] = index;
            }
        }
    }
}
