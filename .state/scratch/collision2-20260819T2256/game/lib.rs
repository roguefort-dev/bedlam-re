...
pub enum GameError {
    ...
    /// The movie stream declares a feature the playback path does not
    /// support (scaled rasters, oversized rasters, Bink audio, or an
    /// audio track off the native mono 8-bit 11025 Hz format).
    #[error("movie feature unsupported: {what}")]
    UnsupportedMovie { what: String },
}
