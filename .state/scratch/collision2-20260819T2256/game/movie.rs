...
impl MoviePlayer {
    ...
    fn collect_audio(&mut self) {
        let eligible = self
            ...
            .audio
            .iter()
            .position(|t| t.is_some_and(|m| m.codec != SmkAudioCodec::Bink
                && m.channels == 1
                && m.bitdepth == 8
...
mod tests {
    ...
    fn synth_stream() -> Vec<u8> {
        ...
        for i in 0..7u32 {
            d.extend_from_slice(&(if i == 0 { 16 } else { 0 }).to_le_bytes());
        }
        ...
        for i in 0..7u32 {
            d.extend_from_slice(&(if i == 0 { 0x4000_0000 | 11_025 } else { 0 }).to_le_bytes());
        }
    ...
    fn finished_player_holds_the_last_frame_and_ignores_dt() {
        ...
        assert_eq!(p.advance(1_000_000), Ok(()));
        assert_eq!(p.frame_index(), 1);
