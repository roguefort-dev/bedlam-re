...
## Now
1. [READY] [id=p5-critter-state-g2-shooters] [gate=p5-critter-state-g2-shooters] P5
...
2. [READY] [id=p5-critter-state-g2-chasers] [gate=p5-critter-state-g2-chasers] P5
   ...
   no Ghidra run): the .NME S3 loader walk for kind 3 (the
   ...
   stage_critters section 3 acceptance + the controller in
   ...
   (ZONEA/M1 hosts no S3) unless a row changes (then documented +
...
1. DONE (2026-08-28, worker cef2f815 claim 1, commits 51933bd +
   ...
   unit). Queued: items 1-2 above (the G2 Shooters unit is the new
   head; the G2 Chasers unit second — the zone-parity surface has
   narrowed to G2).
1. DONE (2026-08-28, worker 05e14378 claim 1, commits a5c3a71 +
   ...
   Shooters unit (both consumed above — the G3 unit is this Done
   entry, the Shooters unit is the queue head). Watchdog repair
   1007791 (2026-08-28): this worker was grace-killed at the 240s
   boundary while re-verifying a51d4f2 — the gates-validator
   battery runs after the bookkeeping commit by contract — so the
   repair landed the push, archived the false preflight-mismatch
   failure, and widened the boundary grace 240 to 900 in this
   repair commit.
