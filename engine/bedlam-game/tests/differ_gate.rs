...
fn s0_s1_cross_and_double_run() {
    ...
    for (id, frames_total, pinned_chain, expect_coverage) in [
        ("S0", 3u64, "c766cc682b73a32c", 0u64),
        ("S1", 401u64, "ed7deab5e3df5ba8", 1u64),
        ("S2", 17u64, "dfb8e457003e36f6", 1u64),
        ...
        ("S3", 133u64, "88e5d849cfb91c09", 1u64),
        ...
        ("S5", 16u64, "0e844d916452ee78", 1u64),
        ("S5B", 19u64, "288f9d39f602bd82", 1u64),
...
fn s1_o2_tiebreak_arbitration() {
    ...
    let e_run = run_canonical(&src, &root).unwrap();
    assert_eq!(
        e_run.manifest.chain_digest, "ed7deab5e3df5ba8",
...
fn s0_o2_transcript_stitch_channel_rule() {
    ...
    let e_run = run_canonical(&src, &root).unwrap();
    assert_eq!(
        e_run.manifest.chain_digest, "c766cc682b73a32c",
