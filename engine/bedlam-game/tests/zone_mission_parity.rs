...
//! mission. This instantiation is ZONE B (§8): missions 1–5 stage
//! through the CAMPAIGN episode-slot seam (the completion mask
//! whose first-uncompleted sub is the mission), missions 6–7 —
//! the MP-only files — through the SELECT write-pair seam
//! (§7j.73/D183). Per criterion, this file's machine-checkable
//! half:
//!
//! | §1 | Criterion | This file's leg |
//! |----|-----------|-----------------|
//! | 1 | scripted flows complete crash-free | `zone_scripted_flows_complete_crash_free` — the zone's committed flows (S5/S5B/S5C, ZONEB/MISSION1) PLUS a generated per-mission battery (boot→mission, passive steady-state, full-staging destroy+pickup+platforms+critters) for EVERY mission: full declared frame budget, dump verifies, two-run byte identity |
//! | 2 | T1 rules vs RE | `zone_t1_rules_spot` (the shared selection/economy arithmetic + the per-mission fetch chain, the zone-level CGR/BIN/LNK pin of D184) + `zone_anchor_ts_statics_rederived_from_tot` (the anchor TS/T0 statics re-derived INDEPENDENTLY from the TOT header bytes + the §7j.64 formula, never the engine's own output) |
...
//! | 6 | original SAVED/OPTIONS.BDL import read-only, bounds-checked, fuzzed | `zone_saved_import_seam_stages_the_shipped_campaign` — the shipped SAVED.BDL slot 0 campaign IS ZONEB/MISSION1 (zone cell 2): import stages the zone's mission 1; empty slots reject loud; bounded deterministic fuzz Ok/Err only |
...
/// THIS suite's zone: **B** (P5-ZONE-GATES §8, the first 7-mission
/// disposition). A later zone's unit instantiates its own const and
/// gate — the tests below never read anything else zone-specific.
const ZONE: ZoneSpec = ZoneSpec {
    letter: 'B',
    missions: 1..=7,
...
fn linear_for(mission: u8) -> u32 {
    let zone = i32::from(ZONE.letter as u8) - i32::from(b'A') + 1;
    (5 * (zone - 2) + i32::from(mission) - 1).clamp(1, 26) as u32
}
...
/// The generated scenario source for one (mission, flow) pair
/// (grammar v1.8: the `mission` key selects the staging seam —
/// 1..=5 the campaign mask, 6..=7 the SELECT MP pair).
fn flow_source(mission: u8, flow: &FlowSpec) -> String {
    let mut s = format!(
        "# generated zone-parity flow: ZONE{L}/MISSION{m} (battery {tag} — the P5 §8 \
...
fn zone_scripted_flows_complete_crash_free() {
    ...
    // The zone's committed flows (staged at mission 1).
    ...
    // The generated battery for EVERY shipped mission of the zone:
    // 1..=5 through the campaign episode-slot seam, 6..=7 through
    // the SELECT MP write-pair seam (the grammar's mission key).
    for mission in ZONE.missions {
        for flow in &FLOWS {
            let id = format!("P5{}M{}{}", ZONE.letter, mission, flow.tag);
            assert_flow_completes(&id, &flow_source(mission, flow), u64::from(flow.frames));
        }
    }
}
...
fn zone_anchor_ts_statics_rederived_from_tot() {
    ...
    for mission in ZONE.missions {
        ...
        let tot = fs::read(root().join(format!("EDITOR/ZONE{}/MISSION{mission}.TOT", ZONE.letter)))
        ...
        assert_eq!(
            (w, h),
            ZONE.dims,
            "MISSION{mission}: TOT header dims (the §2 zone table)"
        );
        assert_eq!(
            tot.len(),
            (4 + 16 * u32::from(w) * u32::from(h)) as usize,
            "MISSION{mission}: 4 + 16·w·h"
        );

        // The anchor frame's TS/T0 statics against values derived
        // from those bytes + the §7j.64 formula — never the engine's
        // own loader output.
        let src = flow_source(mission, &FLOWS[0]);
        let run = run_canonical(&src, &root())
            .unwrap_or_else(|e| panic!("MISSION{mission} boot flow: {e}"));
        ...
        assert_eq!(
            ...
            Some(&map_wh[..]),
            "MISSION{mission}: anchor map-wh == the TOT header dims"
        );
        ...
        assert_eq!(
            anchor.watch("zone"),
            Some(&(zone_index() as u32).to_le_bytes()[..])
        );
        ...
        assert_eq!(
            ...
            Some(&linear_for(mission).to_le_bytes()[..]),
            "MISSION{mission}: linear == clamp(5·(zone−2)+m−1, 1, 26)"
        );
    }
}
...
fn zone_t1_rules_spot() {
    ...
    // The per-mission fetch chain: 25 names, the mission-level
    // path-1 trio + MRK, the ZONE-LEVEL path-2 CGR/BIN/LNK (the
    // D184 no-swap pin — even where a mission-number variant bank
    // ships, MISSION6.BIN is runtime-dead editor residue), and the
    // GAMEGFX staging tail.
    for mission in ZONE.missions {
        let names = mission_asset_names(zone_index(), i32::from(mission));
    ...
    let stage = u8::try_from(zone_index() + 1).expect("stage");
    for mission in 1u8..=5 {
        ...
        assert_eq!(
            host.mission_slot(),
            (zone_index(), i32::from(mission)),
            "the campaign mask selects MISSION{mission}"
        );
        assert_eq!(
    ...
    for (cell, file) in [(1u8, 6u8), (2, 7)] {
        ...
        assert_eq!(
            host.mission_slot(),
            (zone_index(), i32::from(file)),
            "the SELECT pair +5 selects MISSION{file}"
        );
        assert_eq!(
            host.mission_asset_names()[0],
    ...
    }
    {
        ...
        assert_eq!(host.mission_slot(), (zone_index(), 1));
    }
}

// ---------------------------------------------------------------------
// §1 criterion 6 — the original SAVED/OPTIONS.BDL import seam over
// the REAL shipped files (RE-EXW-SIM §7j.70). The shipped slot-0
// campaign IS this zone's mission 1 (zone cell 2 → ZONEB/MISSION1)
// — the zone-B disposition's own import evidence.
// ---------------------------------------------------------------------
...
fn zone_saved_import_seam_stages_the_shipped_campaign() {
    ...
    // Slot 0 is the live campaign ("PLAYER"): zone cell 2 → THIS
    // zone's stage, mask 0 → MISSION1 — the import's staging is the
    // zone-B disposition's own seam evidence.
    let mut host = host();
    ...
    assert_eq!(
        host.mission_asset_names()[0],
        format!("ZONE{}/MISSION1.TOT", ZONE.letter)
    );
