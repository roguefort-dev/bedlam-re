...
const FAMILIES: &[&str] = &[
    "pal", "trn", "bin", "cgr", "min", "lnk", "mrw", "map", "tot", "col", "dat", "trt", "mrk",
    "pos", "pad", "pth", "raw", "smk", "bdl", "nme", "bdg",
];
...
fn corpus_parses_rebuilds_and_round_trips() {
    ...
    for p in &sel {
        ...
        match ext.as_str() {
            ...
            "mrw" => {
                let _ = assets::misc::parse_mrw(&data);
                ok += 1;
            }
...
fn corpus_free_fuzz_no_panics() {
    ...
    for len in sizes {
        ...
        // and a few random w/h through the codecs directly
