...
const TABLE: &[(&str, Facts)] = &[
    ...
    (
        "END.SMK",
        (640, 480, 348, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "GAMEOVER.SMK",
        (640, 480, 84, 66_660, false, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "GTLOG_UK.SMK",
        (640, 480, 70, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "GTLOG_US.SMK",
        (640, 480, 70, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ...
    (
        "LOGO_UK.SMK",
        (640, 480, 71, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "LOGO_US.SMK",
        (640, 480, 71, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ...
    (
        "SHOP.SMK",
        (640, 480, 61, 25_000, true, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "TITLE.SMK",
        (640, 320, 1227, 66_660, false, Some(SmkAudioCodec::Dpcm)),
    ),
    (
        "ZONEDONE.SMK",
        (640, 480, 75, 66_660, true, Some(SmkAudioCodec::Dpcm)),
    ),
];
...
fn smk_corpus_inventory_is_pinned() {
    ...
    let mut files: Vec<&str> = std::fs::read_dir(&dir)
        ...
        .filter(|e| {
            e.path()
                .extension()
                .is_some_and(|x| x.eq_ignore_ascii_case("smk"))
        })
    ...
    for (name, facts) in TABLE {
        ...
        match (codec, info.audio[0]) {
            ...
            (expected, got) => {
                panic!("{name}: track 0 mismatch: table {expected:?} vs corpus {got:?}")
            }
        }
        for t in 1..7 {
