...
fn main() -> ExitCode {
    ...
    let scen_path = match args.next() {
        ...
        None => return die("usage: dbx-stitch <scenario.scen> <capture.dbxcap> --build <binary> [--out-dir DIR] [--pin k=v] [--build-sha256 <hex>]"),
    };
    ...
    let mut header = DumpHeader::new(Channel::O1ExdDosboxX, build_sha256, scenario.id.clone());
    for p in pins {
        header.push_pin(p);
    }
