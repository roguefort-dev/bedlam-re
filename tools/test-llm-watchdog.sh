...
MOCK_CHECK=repair MOCK_REPAIR_COMMIT=1 env "${common[@]}" LLM_WATCHDOG_LOCK="$TMP/repair.lock" "$ROOT/tools/llm-watchdog.sh"
grep -q "zai-coding-plan/glm-5.3#high" "$TMP/calls"
grep -q -- "--agent build" "$TMP/calls"
[ ! -e "$PLAN/.state/PAUSE" ]
[ ! -e "$PLAN/.state/llm-watchdog-pause" ]
grep -q "repair (glm-5.3) produced evidence" "$PLAN/.state/llm-watchdog.log"
grep -q "^state=repaired$" "$PLAN/.state/llm-watchdog-verdict"

# A check observation failure (invalid marker, transport, timeout) must NOT
# stop workers, invoke repair agent, or write a cooldown. State becomes unknown.
repair_calls=$(grep -c "glm-5.3" "$TMP/calls")
notify_lines=$(wc -l < "$TMP/notifications" 2>/dev/null || echo 0)
MOCK_CHECK=invalid env "${common[@]}" LLM_WATCHDOG_LOCK="$TMP/invalid.lock" "$ROOT/tools/llm-watchdog.sh"
[ "$(grep -c "glm-5.3" "$TMP/calls")" -eq "$repair_calls" ]
...
MOCK_CHECK=transport env "${common[@]}" LLM_WATCHDOG_LOCK="$TMP/invalid2.lock" "$ROOT/tools/llm-watchdog.sh"
[ "$(grep -c "glm-5.3" "$TMP/calls")" -eq "$repair_calls" ]
[ "$(wc -l < "$TMP/notifications")" -eq "$((notify_lines + 1))" ]

# repair agent cooldown suppresses only repair, never check observation.
echo $(( $(date +%s) + 600 )) > "$PLAN/.state/llm-watchdog-cooldown-until"
check_calls=$(grep -c "deepseek-v4-flash" "$TMP/calls")
MOCK_CHECK=repair env "${common[@]}" LLM_WATCHDOG_LOCK="$TMP/cool.lock" "$ROOT/tools/llm-watchdog.sh"
[ "$(grep -c "deepseek-v4-flash" "$TMP/calls")" -eq "$((check_calls + 1))" ]
[ "$(grep -c "glm-5.3" "$TMP/calls")" -eq "$repair_calls" ]
[ ! -e "$PLAN/.state/PAUSE" ]
grep -q "check (deepseek-v4-flash-free) still observing" "$PLAN/.state/llm-watchdog.log"
...
repair_calls=$(grep -c "glm-5.3" "$TMP/calls")
MOCK_CHECK=race env "${common[@]}" LLM_WATCHDOG_LOCK="$TMP/race.lock" "$ROOT/tools/llm-watchdog.sh"
...
MOCK_CHECK=repair MOCK_REPAIR_SLEEP=1 env "${common[@]}" REPAIR_TIMEOUT=1 LLM_WATCHDOG_LOCK="$TMP/timeout.lock" "$ROOT/tools/llm-watchdog.sh"
[ ! -e "$PLAN/.state/PAUSE" ]
[ -e "$PLAN/.state/llm-watchdog-cooldown-until" ]
...
MOCK_CHECK=repair MOCK_REPAIR_SLEEP=1 env "${common[@]}" REPAIR_TIMEOUT=10 LLM_WATCHDOG_LOCK="$TMP/signal.lock" "$ROOT/tools/llm-watchdog.sh" &
watchdog_pid=$!
...
resume_holder=$!
MOCK_CHECK=repair MOCK_REPAIR_COMMIT=1 env "${real[@]}" LLM_WATCHDOG_LOCK="$TMP/resume-ok.lock" "$ROOT/tools/llm-watchdog.sh"
...
MOCK_CHECK=repair MOCK_REPAIR_COMMIT=1 env "${real[@]}" RESUME_WAIT_LOOPS=2 RESUME_WAIT_SLEEP=1 LLM_WATCHDOG_LOCK="$TMP/resume-fail.lock" "$ROOT/tools/llm-watchdog.sh"
